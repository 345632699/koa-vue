	...
	<div class="page template-create">
		<div class="header-container">
			<header class="header">
				<el-breadcrumb separator=">">
					...
					<el-breadcrumb-item><a href="/">模板编辑</a></el-breadcrumb-item>
				</el-breadcrumb>
			</header>
		...
		<div class="form-container">
			<el-form ref="form" label-width="80px">
				...
				<el-form-item>
					<el-button type="primary" class="confirm-btn" @click="create">确认修改</el-button>
					<el-button class="return-btn" @click="cancelEdit">取消</el-button>
				</el-form-item>
			</el-form>
		</div>
		<!--字段穿件弹窗-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async handleClose (tag) {
        this.$confirm('此操作将删除该字段, 是否继续?', '提示', {
          ...
          let res = await axios.delete('/api/deleteColumn', params)
          if (res.data.ret) {
            this.$notify.error({
              title: '错误',
      ...
      async create () {
        ...
        let res = await axios.post('/api/updateTemplate', postData)
        if (res.data.ret === 0) {
          this.$notify({
            title: '成功',
            message: res.data.errorMsg,
    ...
    async mounted () {
      ...
      let columnRes = await axios.get('/api/getColumnList', params)
      this.form.template_name = columnRes.data.data.name
      this.original.template_name = columnRes.data.data.name
      let templateColumns = columnRes.data.data.Columns
      templateColumns.forEach(function (item) {
        that.column_list[item.code].styleType = ' '
        that.original.template_ids.push(item.id)
        if (item.id > 9) {
          that.additional_field_list.push(item)
          that.template_ids.push(item.id)
        } else {
          that.has_select_column[item.code] = item
          that.template_ids.push(item.id)
        }
      })
    }
